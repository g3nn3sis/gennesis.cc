...
document.addEventListener('DOMContentLoaded', () => {
  ...
  let startTextContent = startMessage;
  let startIndex = 0;
  ...
  function typeWriterStart() {
    ...
    startText.textContent = startTextContent;
  ...
  const targetSequence = ['e', 'x', 'p', 'o', 'k'];
  let currentIndex = 0;
  document.addEventListener('keydown', (event) => {
    ...
    if (key === targetSequence[currentIndex]) {
      ...
      if (currentIndex === targetSequence.length) {
        window.location.href = 'https://www.expok.xyz';
      }
    } else {
      currentIndex = key === targetSequence[0] ? 1 : 0;
    }
  });